...
function DrawerMenu({ isLogin, children }) {
  ...
  async function barang() {
    try {
      if (isLogin) {
        ...
        navigate("/signin");
        await handleLogout(); // Assuming handleLogout is asynchronous
      }
    } catch (error) {
  ...
  const user = () => {
    if (isLogin) {
      navigate("/user", { state: { userData: isLogin } });
    } else {
      return navigate("/signin");
    }
  };
  ...
  return (
    <div className="flex">
      ...
      <div className="bg-white h-fit mt-4 w-full mr-5 p-2 rounded-md shadow-lg">
        {children}
      </div>
    </div>
  );
